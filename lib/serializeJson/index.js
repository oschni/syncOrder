...
module.exports.setConfig = (conf) => {
    config = conf
    archivePath = config.serialization.archivePath
...
module.exports.archive = function archive(tableId, orders, meta, paied) {
    const archiveFilePath = path.join(archivePath, 'archive.json')
    const data = require(archiveFilePath) || []
    ...
    saveJsonToFile(archiveFilePath, data)
}

module.exports.getStatisticsData = (req, res, next) => {
    const archive = require(config.serialization.archivePath)
    res.statisticData = archive
    next()
}